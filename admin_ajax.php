...
$sql_p = "SELECT admin.*,role.position,branch.name AS branch_name FROM admin 
          ...
          INNER JOIN branch ON admin.b_id = branch.id WHERE admin.name LIKE '%$search%' ORDER BY admin.id ASC";
...
while ($data = mysqli_fetch_assoc($res_p)) {
   $tbl_data .= "<tr>";
    ...
    $tbl_data .= "<td>" . $data['name'] . "</td>";
    $tbl_data .= "<td>" . $data['email'] . "</td>";

    $tbl_data .= "<td>" . $data['position'] . "</td>";
    $tbl_data .= "<td>" . $data['branch_name'] . "</td>";
   $tbl_data .= "<td>
                <div style='height:70px; width: 70px; overflow:hidden;'>
                    <img src='image/admin/" . $data['a_image'] . "' alt='Profile Picture' style='height: 100%; width:100%; object-fit:cover;'>
                </div>
             </td>";
    $tbl_data .= "<td class='actions'><a href='view_admin.php?id=" . $data['id'] . "' class='btn bg-gradient-danger' style='margin-right: 3px;'>DELETE</a>";
    $tbl_data .= "<a href='add_admin.php?id=" . $data['id'] . "' class='btn bg-gradient-primary'>EDIT</a></td>";
