...
echo $search = isset($_GET['search']) ? $_GET['search'] : '';

echo $sql_p = "SELECT * FROM course WHERE course like '%$search%' LIMIT $limit OFFSET $offset";die;
...
while ($data = mysqli_fetch_assoc($res_p)) {
   $tbl_data .= "<tr>";
    ...
    $tbl_data .= "<td>" . $data['course'] . "</td>";
    $tbl_data .= "<td class='actions'><a href='view_course.php?id=" . $data['id'] . "' class='btn bg-gradient-danger' style='margin-right: 3px;'>DELETE</a>";
    $tbl_data .= "<a href='view_course.php?id=" . $data['id'] . "' class='btn bg-gradient-primary'>EDIT</a></td>";
