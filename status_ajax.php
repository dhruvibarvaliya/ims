...
$sql_p = "SELECT * FROM status WHERE status like '%$search%'";
...
while ($data = mysqli_fetch_assoc($res_p)) {
   $tbl_data .= "<tr>";
    ...
    $tbl_data .= "<td class='actions'><a href='view_status.php?id=" . $data['id'] . "' class='btn bg-gradient-danger' style='margin-right: 3px;'>DELETE</a>";
    $tbl_data .= "<a href='view_status.php?id=" . $data['id'] . "' class='btn bg-gradient-primary'>EDIT</a></td>";
